...
// ── ADMIN: сброс анкеты участника (для тестирования регистрации) ──
app.post('/api/admin/reset-profile', requireAuth, requireAdmin, (req, res) => {
  ...
  // Удаляем запись студента — при следующем входе создастся заново без анкеты
  db.prepare('DELETE FROM students WHERE telegram_id = ?').run(String(telegram_id));
  res.json({ ok: true, message: 'Анкета сброшена. При следующем входе пользователь заполнит её заново.' });
});
...
app.listen(PORT, () => {
  // TEMP: сброс для тестирования регистрации — удалить после теста
  try {
    db.prepare("DELETE FROM students WHERE telegram_id = '7385674488'").run();
    db.prepare("UPDATE users SET active_token = '[]' WHERE telegram_id = '7385674488'").run();
    console.log('🔄 Тест-сброс: анкета и сессия удалены для 7385674488');
  } catch(e) {}
  // END TEMP
